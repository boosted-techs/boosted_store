...
{block name="body"}
    <h4 class="p-4 text-center">Products</h4>
    ...
    <table class="table table-responsive">
        ...
        {foreach $products item=list}
            <tr class="{if $list.status == 0} alert alert-danger{/if}">
                ...
                <td>{if $list.feature_photo} <img src="//{$smarty.server.SERVER_NAME}/assets/loading.gif" lsrc="//{$smarty.server.SERVER_NAME}/media/products_thumbnails/{$list.feature_photo}" class="img-thumbnail" style="width:200px;"/>{else}<b>No photo</b>{/if}</b></td>
                <td>
                    <a href="//{$smarty.server.SERVER_NAME}/dashboard/{$list.url}">{$list.name}</a> <br/>
                    <a href="//{$smarty.server.SERVER_NAME}/duplicate/{$list.id}" title="Duplicate item"><i class="fa fa-copy"></i></a>
                <br/>
